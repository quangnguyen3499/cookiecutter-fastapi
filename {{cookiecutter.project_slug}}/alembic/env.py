...
from configs.database.db import metadata
...
db_driver = settings.DATABASE_URI.scheme
db_driver_parts = db_driver.split("+")
if len(db_driver_parts) > 1:  # e.g. postgresql+asyncpg
    sync_scheme = db_driver_parts[0].strip()
    DATABASE_URI = DATABASE_URI.replace(db_driver, sync_scheme)  # replace with sync driver

logger.debug(f"DATABASE: {DATABASE_URI}")
